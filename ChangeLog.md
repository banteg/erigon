...
- rpc: `trace_filter` now defaults to intersection: OR within each address list, AND between `fromAddress` and `toAddress` when both are set; an omitted, `null` or empty list imposes no restriction. Queries that set both lists without `mode` previously returned the union; pass `mode: "union"` to keep that behavior. `mode` values other than `"union"` and `"intersection"`, including `""` and `null`, now return `-32602` instead of falling back to union. This can be a breaking change (#24255) — by @banteg
